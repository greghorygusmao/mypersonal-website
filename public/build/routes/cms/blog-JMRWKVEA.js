...
function useVirtual(_ref) {
  ...
  var _useMeasureParent = useMeasureParent(parentRef2, initialRect), outerSize = _useMeasureParent[sizeKey];
  latestRef.current.outerSize = outerSize;
  var defaultScrollToFn = import_react.default.useCallback(function(offset) {
    if (parentRef2.current) {
      parentRef2.current[scrollKey] = offset;
    }
  }, [parentRef2, scrollKey]);
  ...
  var element = onScrollElement ? onScrollElement.current : parentRef2.current;
...
var useVirtualList = ({ size, items = [], service, cursorItems }) => {
  const parentRef2 = (0, import_react3.useRef)(canUseDOM() ? window.document.body : null);
...
var Items = ({ children, service, items }) => {
  const [isLoading, rowVirtualizer] = useVirtualList({ service, items });
...
//# sourceMappingURL=/build/routes/cms/blog-JMRWKVEA.js.map
