...
var useVirtualList = ({ size, items = [], service, cursorItems }) => {
  const parentRef = (0, import_react2.useRef)(canUseDOM() ? window.document.body : null);
  const [cursor, setCursor] = (0, import_react2.useState)(cursorItems);
  const [itemsInner, setItemsInner] = (0, import_react2.useState)(items);
  const rowVirtualizer = useVirtual({
    size: cursor?.hasNextPage ? itemsInner?.length + 1 : itemsInner?.length,
    parentRef,
    estimateSize: (0, import_react2.useCallback)(() => size, [])
  });
...
var Items = ({ children, service, items }) => {
  const [isLoading2, rowVirtualizer] = useVirtualList({ service, items });
...
//# sourceMappingURL=/build/routes/cms/blog-G7GJ4GJ7.js.map
